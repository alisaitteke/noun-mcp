...
release_notes_repo="${GITHUB_REPOSITORY:-alisaitteke/noun-mcp}"

# previous_version_tag TAG → semver predecessor tag (e.g. v1.0.3 → v1.0.2), or empty.
previous_version_tag() {
  local tag="$1"
  git tag -l 'v*' --sort=-v:refname | awk -v t="$tag" 'found { print; exit } $0 == t { found = 1 }'
}

# next_version_tag TAG → semver successor tag (e.g. v1.0.2 → v1.0.3), or empty.
next_version_tag() {
  local tag="$1"
  git tag -l 'v*' --sort=-v:refname | awk -v t="$tag" '$0 == t { getline; print; exit }'
}

# changelog_anchor 1.0.4 → GitHub-compatible heading anchor fragment.
changelog_anchor() {
  local version="$1"
